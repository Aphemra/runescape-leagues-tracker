...
export default function PlayerStatsModal({ manifest, stats, onChange, onClose }: PlayerStatsModalProps) {
  const groups = useMemo(() => [...new Set(manifest.playerStats.map((stat) => stat.group))], [manifest.playerStats]);
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="player-stats-title">
                        ...
                        onChange={(event) => {
                          ...
                          onChange(
                            ...
                            Number.isFinite(next)
                              ? Math.min(stat.maximum, Math.max(stat.minimum, Math.floor(next)))
                              : stat.minimum,
                          );
                        }}
                      />
        ...
        <footer className="modal__footer">
          <span>
            Total level <strong>{total.toLocaleString()}</strong>
          </span>
